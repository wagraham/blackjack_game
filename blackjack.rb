...
# D. the player & dealer must take turns, and either hit or stay. 
...
while mytotal < 21
  ...
  #hit < no if condition to simplify things. 
  new_card = deck.pop
...
while dealertotal < 17 
  ...
  puts "Dealer hits: #{new_card}" # local variable within method score can be reused
  dealercards << new_card
  dealertoral = calculate_total(dealercards)
  puts "Dealer total is now: #{dealertoral}"

  if dealertoral == 21
    Puts "Dealer hit Blackjack."

  elsif dealertoral > 21
...
if dealertoral > mytotal
  puts "Sorry, dealer won"
elsif dealertoral < mytotal
  puts "You win!"
else 
  puts "It's a tie."
end
